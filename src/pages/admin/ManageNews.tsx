...
type Article = Tables<"articles"> & { status?: string };
...
const ManageNews = () => {
  ...
  imagesRef.current = articleImages;
  ...
  const saveArticle = useCallback(async (status: string, silent = false) => {
    const payload: any = buildPayload(status);
    let articleId = editingRef.current?.id;

    if (editingRef.current) {
      const { error } = await supabase.from("articles").update(payload).eq("id", editingRef.current.id);
      if (error) {
      ...
      articleId = data.id;
      // Set editing to the new article so subsequent saves update instead of insert
      setEditing({ ...payload, id: articleId, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), is_popular: false } as Article);
    }
    ...
    if (articleId) {
      ...
      const imgs = imagesRef.current;
      if (imgs.length > 0) {
        const imgPayload = imgs.map((img, i) => ({
  ...
  useEffect(() => {
    if (dialogOpen) {
      autoSaveTimerRef.current = setInterval(async () => {
        if (hasUnsavedChanges.current && formRef.current.title.trim()) {
          await saveArticle("draft", true);
          toast({ title: "Draft auto-saved", description: "Your changes have been saved automatically." });
        }
      }, AUTO_SAVE_INTERVAL);
    } else {
  ...
  }, [dialogOpen, saveArticle]);

  // Save draft on page unload
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (dialogOpenRef.current && hasUnsavedChanges.current && formRef.current.title.trim()) {
        // Use sendBeacon for reliable save on unload
        const payload = buildPayload("draft");
        const id = editingRef.current?.id;
        // We can't reliably do async on unload, so just warn
        e.preventDefault();
        e.returnValue = "You have unsaved changes. They will be saved as a draft.";
      }
    };
  ...
  const openEdit = async (a: Article) => {
    ...
    setArticleImages(
      (imgs ?? []).map((img: any) => ({
        id: img.id, image_url: img.image_url, caption: img.caption ?? "",
        caption_en: img.caption_en ?? "", sort_order: img.sort_order, is_cover: img.is_cover,
      }))
    );
    hasUnsavedChanges.current = false;
    setDialogOpen(true);
  ...
  // Auto-save draft when dialog closes
  const handleDialogChange = async (open: boolean) => {
    if (!open && hasUnsavedChanges.current && formRef.current.title.trim()) {
      await saveArticle("draft", true);
      toast({ title: "Draft saved", description: "Your changes were auto-saved as a draft." });
      fetchData();
    }
    if (!open) hasUnsavedChanges.current = false;
    setDialogOpen(open);
  };
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <MultiImageUploader
              ...
              onChange={(imgs) => { setArticleImages(imgs); hasUnsavedChanges.current = true; }}
              maxImages={3}
              showCoverBadge

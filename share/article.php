...
// This keeps your SPA build, but gives WhatsApp/Facebook bots real OG tags.
...
{
    return preg_match(
        '/whatsapp|facebookexternalhit|facebot|twitterbot|linkedinbot|telegrambot|slackbot|discordbot|bot|crawler|spider|pinterest|redditbot|googlebot|bingbot/i',
        $ua
    ) === 1;
}
...
$articleArr = sb_get_json(
    ...
    rawurlencode($articleId) .
    '&select=id,title,title_en,description,description_en,thumbnail_url',
    $SUPABASE_ANON_KEY
);

$article = (count($articleArr) > 0 && is_array($articleArr[0]))
    ? $articleArr[0]
    : null;
...
$imgArr = sb_get_json(
    $SUPABASE_URL .
    '/rest/v1/article_images?article_id=eq.' .
    rawurlencode($articleId) .
    '&select=image_url&order=sort_order&limit=1',
    $SUPABASE_ANON_KEY
);
...
} elseif (isset($article['thumbnail_url'])) {
    $ogImage = (string) $article['thumbnail_url'];
}


/* Convert Supabase storage URL to optimized rendered image */
